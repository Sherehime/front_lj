...
//https://react-icons.github.io/icons?name=bs     https://github.com/react-icons/react-icons иконки


class Form_forgot_password extends React.Component {

    sendEmail = async (e)=> {
        e.preventDefault();
        const email = e.target.elements.mail.value;
        cogoToast.info( "Письмо с новым паролем будет отправлео на   "+ email, { position: 'top-right'});
        document.getElementById("input_for_forgot_email").value = "";
        ...
        return (
            <div className="wrapper gdBFSQ">
                <form className="wrapper-container " onSubmit={this.sendEmail}>
                    ...
                    <div className={" label-float"} data-validate={"Enter email"}>
                        <input   className={"login_input__form"} id ={"input_for_forgot_email"} type={"email"} name={"mail"} placeholder={"email"} autoComplete={"off"}  />
...
export default Form_forgot_password;
